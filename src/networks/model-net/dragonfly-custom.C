...
// SDN Variables
unordered_map<string, unordered_map<string, vector<int64_t>>> flow_table;
...
void terminal_custom_init(terminal_state *s,
                          tw_lp *lp)
{
  s->packet_gen = 0;
  s->packet_fin = 0;
                            ...
                            tw_lp *lp)
{
  ...
  { // TODO This hardcoded 2 * s->params->cn_vc_size seems dubious
                                  ...
                                  tw_lp *lp)
{
  ...
  if (s->router_id == 0)
  {
    ...
    fprintf(fp, "\n\nOrdering of links \n%d green (router-router same row) channels \n %d black (router-router same column) channels \n %d global (router-router remote group)"
                " channels \n %d terminal channels",
            p->num_router_cols * p->num_row_chans, p->num_router_rows * p->num_col_chans, p->num_global_channels, p->num_cn);
    fclose(fp);
  }
...
void terminal_custom_event(terminal_state *s,
                           tw_bf *bf,
                           terminal_custom_message *msg,
                           tw_lp *lp)
{
...
void dragonfly_custom_terminal_final(terminal_state *s,
                                     tw_lp *lp)
{
              ...
              int do_chan_selection)
{
  ...
  if (msg->last_hop == TERMINAL || s->router_id == msg->intm_rtr_id || (routing == PROG_ADAPTIVE && do_chan_selection))
  {
    if (adap_chan >= 0)
                      ...
                      tw_lp *lp)
{
  ...
  string flow_src = to_string(msg->src_terminal_id) + "-" + to_string(s->router_id);
  string flow_dest = to_string(msg->dest_terminal_id) + "-" + to_string(dest_router_id);
  ...
  if (dest_grp_id != src_grp_id &&
      ((cur_chunk->msg.last_hop == TERMINAL && routing == ADAPTIVE) || (cur_chunk->msg.path_type == MINIMAL && routing == PROG_ADAPTIVE && s->group_id == src_grp_id)))
  {
    bf->c20 = 1;
  ...
  if ((cur_chunk->msg.last_hop == TERMINAL || (routing == PROG_ADAPTIVE && s->group_id == src_grp_id && prev_path_type != next_path_type)) && cur_chunk->msg.path_type == NON_MINIMAL)
  {
  ...
  // if (s->switch_level == 0 && msg->last_hop == TERMINAL)
  {
    ...
    unordered_map<string, unordered_map<string, vector<int64_t>>>::const_iterator src_it = flow_table.find(flow_src);
    if (src_it != flow_table.end())
    {
      /*Flow table has the source*/
      unordered_map<string, vector<int64_t>>::const_iterator dest_it = flow_table[flow_src].find(flow_dest);
    ...
    else
    {
      ...
      vector<int64_t> temp(1, 0);
      temp[0] = data_sent;
      unordered_map<string, vector<int64_t>> inner;
...
static vector<pair<string, string>> get_elephants(unordered_map<string, unordered_map<string, vector<int64_t>>> flows, int interval_time)
{
  ...
  for (auto it = flows.begin(); it != flows.end(); it++)
  {
    for (auto it_2 = it->second.begin(); it_2 != it->second.end(); it_2++)
    {
      string src_term_router = it->first;
      string dest_term_router = it_2->first;
      string src = src_term_router.substr(0, src_term_router.find("-"));
      string dest = dest_term_router.substr(0, dest_term_router.find("-"));
      string src_sw = src_term_router.substr(src_term_router.find("-") + 1);
      string dest_sw = dest_term_router.substr(dest_term_router.find("-") + 1);
...
{
  if (count_switch_update < (ns->params->num_groups * ns->params->num_routers) - 1)
  {
